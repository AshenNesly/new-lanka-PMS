...
// Populate customer details in the UI
function populateCustomerDetails(customer) {
    ...
    document.getElementById('customerDob').textContent = customer.date_of_birth;
    ...
    // Calculated fields
    document.getElementById('customerAge').textContent = customer.age ? `${customer.age} years` : 'N/A';
    document.getElementById('customerRegDate').textContent = customer.reg_date;
    document.getElementById('customerSince').textContent = customer.customer_since || 'N/A';
    
    // Purchase summary
    document.getElementById('totalPurchases').textContent = customer.total_purchases;
    document.getElementById('totalAmountSpent').textContent = `Rs. ${customer.total_spent.toFixed(2)}`;
...
function populatePurchaseHistory(purchases) {
    ...
    if (purchases.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="6" style="text-align: center;">No purchase history found</td></tr>';
        return;
    }
    ...
    purchases.forEach(purchase => {
        ...
        row.innerHTML = `
            <td><a href="admin-inside-sale.html?id=${purchase.sale_id}&date=${purchase.date}&time=${purchase.time}&customerName=${encodeURIComponent(currentCustomer.full_name)}&customerId=${currentCustomerId}&totalMedicines=${purchase.items}&amount=Rs.%20${purchase.amount.toFixed(2)}&userName=${encodeURIComponent(purchase.user)}" class="medicine-link">${purchase.sale_id}</a></td>
            <td>${purchase.date}</td>
            <td>${purchase.time}</td>
            <td>${purchase.items}</td>
            <td>Rs. ${purchase.amount.toFixed(2)}</td>
            <td>${purchase.user}</td>
        `;
    });
}
