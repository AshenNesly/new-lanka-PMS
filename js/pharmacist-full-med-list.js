 ...
 */
function updateMedicineCount(totalRecords, searchTerm = '') {
    const medicineCountElement = document.querySelector('.table-header h3');
    if (medicineCountElement) {
        if (searchTerm) {
            medicineCountElement.textContent = `Search Results (${totalRecords} items found)`;
        } else {
            medicineCountElement.textContent = `All Medicines (${totalRecords} items)`;
        }
    }
}
...
function showError(message) {
    const tableBody = document.querySelector('.medicine-table tbody');
    if (tableBody) {
        tableBody.innerHTML = `<tr><td colspan="7" style="text-align: center; padding: 40px; color: #dc3545;">${message}</td></tr>`;
    }
    console.error('Error:', message);
}

/**
 * Add medicine to backend using same format as admin
 */
...
// Save medicine with backend integration
modalSave.addEventListener('click', async function() {
    const medicineName = document.getElementById('modalMedicineName').value;
    ...
    const howToUse = document.getElementById('modalHowToUse').value;
    const sideEffects = document.getElementById('modalSideEffects').value;
    ...
    try {
        ...
        // Add medicine via backend using same format as admin
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Pharmacist Full Medicine List page loaded');
    
    // Load medicine groups for the modal dropdown
    loadMedicineGroups();
    
    // Load initial medicines list
    loadMedicinesList(1, '');
});
