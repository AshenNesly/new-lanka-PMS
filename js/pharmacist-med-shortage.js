...
async function fetchShortageMedicines(page = 1, search = '') {
    try {
        ...
        if (data.success) {
            ...
            updatePagination();
            // Use total_records from pagination or fall back to medicines array length
            const totalShortage = data.data.pagination?.total_records || shortageData.length;
            updateShortageCount(totalShortage);
        } else {
...
function displayShortageMedicines(medicines) {
    ...
    tbody.innerHTML = medicines.map(medicine => {
        ...
        // Determine status badge class and text to match admin side
...
// Add CSS for animations only (other styles should come from main CSS file)
