...
function displaySalesInTable(salesData) {
    ...
    salesData.forEach(sale => {
        ...
        // Create the link URL for pharmacist-inside-sale.html instead of admin-inside-sale.html
        const linkUrl = `pharmacist-inside-sale.html?id=${sale.formatted_sale_id}&date=${sale.sale_date}&time=${sale.sale_time}&customerName=${encodeURIComponent(sale.customer_name)}&customerId=${sale.customer_id || ''}&totalMedicines=${sale.total_medicines}&amount=${encodeURIComponent(sale.formatted_amount)}&userName=${encodeURIComponent(sale.user_name)}`;
