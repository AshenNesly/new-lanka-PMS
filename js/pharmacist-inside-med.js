...
function loadFromURLParameters() {
    ...
    const medicineData = {
        ...
        howToUse: urlParams.get('howToUse') || 'No usage instructions available.',
        sideEffects: urlParams.get('sideEffects') || 'No side effects information available.'
    };
...
deleteBtn.addEventListener('click', async () => {
    ...
    if (confirm(`Are you sure you want to delete "${medicineName}"? This action cannot be undone.`)) {
        try {
            ...
            setTimeout(() => {
                window.location.href = 'pharmacist-full-med-list.html';
            }, 2000);
